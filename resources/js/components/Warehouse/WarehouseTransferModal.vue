...
const form = useForm({
    dari_warehouse_id: '' as string | number,
    ke_warehouse_id: '' as string | number,
...
watch(() => props.open, (val) => {
    if (val && props.sourceWarehouse) {
        form.dari_warehouse_id = props.sourceWarehouse.id;
    }
    if (!val) form.reset();
});
...
const sourceData = computed(() => {
    ...
    const stok = Number(props.sourceWarehouse.stok_saat_ini ?? 0);
    const kapasitas = Number(props.sourceWarehouse.kapasitas_maks ?? 0);
    const utilisasi = kapasitas > 0 ? Math.round((stok / kapasitas) * 100) : 0;
    return {
        nama: props.sourceWarehouse.nama,
        stok,
        kapasitas,
        utilisasi,
    };
...
const destinationData = computed(() => {
    if (!form.ke_warehouse_id) return null;
    const wh = props.allWarehouses.find(w => w.id == form.ke_warehouse_id);
    if (!wh) return null;
    const stok = Number(wh.stok_saat_ini ?? 0);
    const kapasitas = Number(wh.kapasitas_maks ?? 0);
    const utilisasi = kapasitas > 0 ? Math.round((stok / kapasitas) * 100) : 0;
    return {
        nama: wh.nama ?? wh.label,
        stok,
        kapasitas,
        utilisasi,
    };
...
const sourceAfter = computed(() => {
    ...
    return sourceData.value.stok - volumeNum.value;
});

const destAfter = computed(() => {
    if (!destinationData.value) return 0;
    return destinationData.value.stok + volumeNum.value;
});
        ...
        <DialogContent class="rounded-2xl sm:max-w-xl p-0 gap-0 overflow-hidden">
            ...
            <div class="px-6 py-5 flex flex-col gap-4 max-h-[75vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div
                            ...
                            <template v-if="sourceData">
                                <span class="truncate">
                                    {{ sourceData.nama }}
                                    <span class="text-gray-400">({{ formatNumber(sourceData.stok) }} kg ·</span>
                                    <span :class="utilisasiColor(sourceData.utilisasi)" class="font-medium">{{
                                        sourceData.utilisasi }}%</span><span class="text-gray-400">)</span>
                                </span>
                            </template>
                            <template v-else>
                        ...
                        <span v-if="form.errors.dari_warehouse_id" class="text-xs text-red-500">{{
                            form.errors.dari_warehouse_id }}</span>
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="relative">
                            <select v-model="form.ke_warehouse_id" :class="selectClass(!!form.errors.ke_warehouse_id)">
                        ...
                        <div v-if="destinationData" class="text-xs text-gray-400">
                            {{ formatNumber(destinationData.stok) }} kg ·
                            <span :class="utilisasiColor(destinationData.utilisasi)" class="font-medium">{{
                                destinationData.utilisasi }}%</span>
                        </div>
                        <span v-if="form.errors.ke_warehouse_id" class="text-xs text-red-500">{{
                            form.errors.ke_warehouse_id }}</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="relative">
                            <input v-model="form.tanggal_transfer" type="date" :class="inputClass" />
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="relative">
                            <input v-model="form.estimasi_tiba" type="date" :class="inputClass" />
                        </div>
                    </div>
                    <div class="flex flex-col gap-1.5">
                        <Label class="text-sm font-medium text-gray-700">Petugas</Label>
                        <input v-model="form.petugas" type="text" placeholder="Nama petugas transfer"
                            :class="inputClass" />
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <textarea v-model="form.catatan" rows="3" placeholder="Alasan / catatan transfer..."
                ...
                <!-- ── Summary Cards ────────────────────────── -->
                <div v-if="sourceData && destinationData && volumeNum > 0"
                    ...
                    <div class="rounded-lg border border-gray-200 bg-gray-50 p-3.5">
                        <p class="text-[11px] font-bold text-gray-500 uppercase tracking-wide mb-2">
                            GUDANG ASAL — {{ sourceData.nama }}
                        </p>
                        <div class="flex flex-col gap-1 text-[13px]">
                            <div class="flex items-center justify-between">
                                <span class="text-gray-500">Stok saat ini</span>
                                <span class="text-gray-800 font-medium">{{ formatNumber(sourceData.stok) }} kg</span>
                            </div>
                            <div class="flex items-center justify-between">
                    ...
                    <div class="rounded-lg border border-[#007C95]/20 bg-[#007C95]/[0.03] p-3.5">
                        <p class="text-[11px] font-bold text-[#007C95] uppercase tracking-wide mb-2">
                            GUDANG TUJUAN — {{ destinationData.nama }}
                        </p>
                        <div class="flex flex-col gap-1 text-[13px]">
                            <div class="flex items-center justify-between">
                                <span class="text-gray-500">Stok saat ini</span>
                                <span class="text-gray-800 font-medium">{{ formatNumber(destinationData.stok) }} kg</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-gray-500">Volume transfer</span>
                ...
                <div v-if="sourceData && volumeNum > 0 && sourceAfter < 0"
                    ...
                    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" class="shrink-0">
                        <path d="M8 5v3m0 2.5h.005M14.5 8a6.5 6.5 0 1 1-13 0 6.5 6.5 0 0 1 13 0Z"
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-white">
                <Button variant="outline"
                    class="rounded-lg border-gray-200 px-5"
                    :disabled="form.processing"
                    @click="emit('update:open', false)">
                    Batal
                </Button>
                <Button
                    :disabled="form.processing || !form.ke_warehouse_id || volumeNum <= 0"
                    class="rounded-lg bg-[#007C95] text-white hover:bg-[#006b80] px-5"
                    @click="handleSubmit">
